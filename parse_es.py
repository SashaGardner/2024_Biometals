...
def ParseInput(ArgsIn):
   UseMsg = "parse_es [options] [target_dir]"
...
def parse_tddft_results(target_dir, options):
   ...
   for line in fr.readlines():
      ...
      if l_sp[1] == 'Excited': #first useful line for a given job
         frame_idx = int(re.search(options.placeholder+"([^_]+)_", l_sp[0]).group(1))
   ...
   for frame_idx in sorted(data_tddft):
       fw = open(f'ex_states_{frame_idx}.csv', 'w')
       fw.write("state,E_ex,osc\n")
